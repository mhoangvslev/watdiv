...
namespace ASSOCIATION_CONSTRAIN_TYPES {
    enum enum_t {
        INDEPENDANT,                               // generate when the condition allows, independant of other association
        IF_PREVIOUSLY_EXISTED_ONLY,         // Generate when the subject is previously generated by another association, 
        PARTIAL,                            // Generate when previous existed and generate when the condition allows
        FULL                                // Like IF_PREVIOUSLY_EXISTED, but only when the generation condition for this association allows.
    };
};
